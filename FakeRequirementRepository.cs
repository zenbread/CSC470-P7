...
namespace P7
{
    class FakeRequirementRepository : IRequirementRepository
    {
        ...
        const string DUPLICATE_STATEMENT_ERROR = "Statements must be unique.";
        const string EMPTY_STATEMENT_ERROR = "Statements must have a value.";
        const string REQUIREMENT_NOT_FOUND_ERROR = "Requirement does not exist.";
        const string MISSING_FEATUREID_ERROR = "Must select a feature for this requirement.";
        const string MISSING_PROJECTID_ERROR = "Must select a project for this requirement.";
        ...
        public FakeRequirementRepository()
        {
            if (requirements.Count() == 0)
            {
                ...
                requirements.Add(new Requirement { Id = GetNextId(), FeatureId = 2, ProjectId = 1, Statement = "Second requirement."});
                requirements.Add(new Requirement { Id = GetNextId(), FeatureId = 3, ProjectId = 1, Statement = "Third requirement."});
            }
        }
        private string DuplicateStatement(string statement)
        {
            string check = NO_ERROR;
            Requirement found = requirements.Find(x => x.Statement == statement);
            if (found != null)
        ...
        public string Add(Requirement requirement)
        {
            ...
            else if (DuplicateStatement(requirement.Statement) != NO_ERROR)
